...
function BrowseContent({ initialData }: { initialData?: BrowseResult }) {
  ...
  const { data, isLoading, error } = useQuery<BrowseResult>({
    ...
    // Seed the first render with the server-rendered results so the initial
    // paint matches the SSR HTML (no flash/refetch). Only applies to the first
    // queryKey; changing filters fetches fresh from /api/browse.
    initialData,
  });
...
export function BrowseClient({
  ...
  return (
    ...
    >
      <BrowseContent initialData={initialData} />
    </Suspense>
  );
}
