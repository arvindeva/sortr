  ...
  return (
    <>
      ...
      <ArcadePageHeader
        ...
        eyebrow="Edit sorter"
        title="Edit sorter"
        subtitle="Update the title, cover, and the things you want to rank."
        actions={
          <Link
            href={`/sorter/${sorter.slug}`}
            className="flex items-center gap-2 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground transition-colors hover:text-foreground"
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          ...
          {tags.length > 0 && (
            <section className="rounded-2xl border border-border bg-card p-6 md:p-7">
              ...
              <p className="mb-5 text-sm text-muted-foreground">
                Manage the filter tags on this sorter. New sorters no longer use
                tags.
              </p>
          ...
          <section className="rounded-2xl border border-border bg-card p-6 md:p-7">
            <div className="mb-5 flex items-center justify-between gap-3">
              ...
              <span
                className={`font-mono text-xs ${
                  fields.length >= 2 ? "text-cyan-ink" : "text-muted-foreground"
                }`}
              >
                {fields.length} items{" "}
                {fields.length >= 2 ? "· ready" : "· min 2"}
              </span>
            </div>
            <div className="mb-4">
              {/* Buttons */}
                      ...
                      render={({ field }) => (
                        <FormItem className="gap-1.5">
                          <div className="flex items-center gap-3 rounded-[10px] border border-border bg-muted px-3 py-2.5">
                            {/* Image preview (with replace) or color thumb */}
                            {(() => {
                              ...
                              <div className="relative flex-shrink-0">
                                <img
                                  ...
                                  alt={`Preview ${index + 1}`}
                                  className="h-8 w-8 rounded-[7px] border border-border object-cover"
                                />
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  className="absolute -right-1 -bottom-1 h-5 w-5 p-0"
                                  onClick={() => {
            ...
            <div className="mt-5 space-y-1 text-sm text-muted-foreground">
              ...
              {managedTags.length > 0 && (
                <p>
                  <strong className="text-foreground">Tags:</strong> Click tag
                  buttons below each item to assign tags. Items without tags
                  will always appear in sorting.
                </p>
              )}
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            >
              <Link href={`/sorter/${sorter.slug}`}>Cancel</Link>
            </Button>
            <Button
              type="submit"
              arcade
