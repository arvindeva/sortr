...
export const createSorterSchema = z.object({
  ...
  items: z.array(
    z.object({
      title: z.string().min(1, "Item title is required").max(100, "Item title must be 100 characters or less"),
...
export const createSorterFormSchema = createSorterSchema.omit({
  items: true
}).extend({
  items: z.array(
    z.object({
