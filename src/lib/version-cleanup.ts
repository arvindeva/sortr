import { db } from "@/db";
import { sorterHistory, sorterItems, sorterGroups, sortingResults } from "@/db/schema";
 ...
 */
export async function cleanupVersion(sorterId: string, version: number): Promise<void> {
  try {
    ...
    if (rankingsUsingVersion.length > 0) {
      console.log(`Version ${version} gained references during cleanup, skipping`);
      return;
    }

    // Safe to delete - no rankings reference this version
    console.log(`Cleaning up unreferenced version ${version} for sorter ${sorterId}`);

    // Delete from database tables
    await db.delete(sorterHistory)
      .where(and(
        eq(sorterHistory.sorterId, sorterId),
        eq(sorterHistory.version, version)
      ));

    await db.delete(sorterItems)
      .where(and(
        eq(sorterItems.sorterId, sorterId),
        eq(sorterItems.version, version)
      ));

    await db.delete(sorterGroups)
      .where(and(
    ...
    console.log(`R2 cleanup for v${version}: ${r2Cleanup.deleted.length} deleted, ${r2Cleanup.preserved.length} preserved`);

  } catch (error) {
    console.error(`Error cleaning up version ${version} for sorter ${sorterId}:`, error);
...
export async function cleanupOrphanedVersions(): Promise<{
  ...
  try {
    ...
    const allVersions = await db
      .select({ sorterId: sorterHistory.sorterId, version: sorterHistory.version })
      .from(sorterHistory);
    ...
    console.log(`Background cleanup completed: ${versionsDeleted}/${versionsChecked} versions cleaned up`);
