...
function RetroBox({
  ...
  size,
  ...props
}: React.ComponentProps<"div"> & VariantProps<typeof retroBoxVariants>) {
  return (
    <div
      data-slot="retro-box"
      className={cn(retroBoxVariants({ variant, size }), className)}
      {...props}
    />
  );
}

export { RetroBox, retroBoxVariants };