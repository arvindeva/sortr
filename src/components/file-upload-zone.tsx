    ...
    async (acceptedFiles: File[], rejectedFiles: any[]) => {
      ...
      if (acceptedFiles.length > 0) {
        ...
        if (enableCompression) {
          ...
          const nonCompressibleFiles = acceptedFiles.filter(
          ...
          if (compressibleFiles.length > 0) {
            ...
            setCompressionProgress({
              completed: 0,
              total: compressibleFiles.length,
            });
        ...
        <CardContent className="p-6">
              ...
              {isCompressing ? (
                <div>
                  ...
                  <p className="text-muted-foreground text-sm">
                    ...
                    {compressionProgress.total} images processed
                  </p>
                </div>
              ) : isDragActive ? (
                <div>
          ...
          {dragError && (
            <div className="bg-destructive/10 border-destructive/20 mt-4 rounded-md border p-3">
              <p className="text-destructive text-sm">{dragError}</p>
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <CardContent className="p-4">
            <div className="space-y-2">
              <h4 className="text-sm font-medium">
                Selected Files ({existingFiles.length})
              </h4>
