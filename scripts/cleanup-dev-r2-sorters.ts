...
if (R2_ENDPOINT) {
  const match = R2_ENDPOINT.match(/https:\/\/([a-f0-9]+)\.r2\.cloudflarestorage\.com/);
...
function isValidDevBucket(bucketName: string): { valid: boolean; reason?: string } {
  ...
  const foundProdIndicator = prodIndicators.find(indicator => nameLower.includes(indicator));
  ...
  if (!validPatterns.some(pattern => pattern)) {
    return { 
      valid: false, 
      reason: "Must end with '-dev', start with 'dev-', contain '.dev', 'development', 'staging', 'test', or be named 'dev'" 
    };
  }
  ...
  try {
    ...
    for (const folder of foldersToClean) {
      ...
      for (let i = 0; i < keys.length; i += BATCH_SIZE) {
        ...
        console.log(`🔄 Processing ${folder} batch ${batchNum}/${totalBatches} (${batch.length} objects)...`);
