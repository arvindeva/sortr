...
async function cleanupAll() {
  try {
    ...
    if (allSuccessful) {
      ...
      console.log(`\n📊 What was cleaned:`);
      console.log(`  🗄️  Database tables: sessionFiles, sorterGroups, sorterHistory, sorterItems, sorters, sortingResults, uploadSessions`);
      console.log(`  📁 R2 folders: sessions/, sorters/`);
      console.log(`\n🛡️  What was preserved:`);
      console.log(`  👤 Database: user, account, session, verificationToken tables`);
      console.log(`  🖼️  R2 storage: avatars/ folder`);
      console.log(`\n✅ Your dev environment is now clean and ready for fresh testing!`);
    } else {
      console.log(`❌ Cleanup sequence failed. Some operations may have completed successfully.`);
...
function askForConfirmation(): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    console.log("⚠️  WARNING: This will permanently delete all sorter data from your dev environment!");
    console.log("🛡️  Users and avatars will be preserved.");
    console.log("");
    
    rl.question("Are you sure you want to continue? (type 'yes' to confirm): ", (answer: string) => {
      rl.close();
